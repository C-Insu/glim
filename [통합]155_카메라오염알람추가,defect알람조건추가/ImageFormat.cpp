...
public:
	EncoderInfo()
	{
		...
		for (unsigned int j=0; j<num; ++j) {
			...
			if (wcsicmp(_type.c_str(), L"jpeg") == 0) {
				m_clsid_map.insert(
	...
	struct nocase_comparator : public std::binary_function<const std::wstring, const std::wstring, bool>
	{
		...
		{
			return (wcsicmp(x.c_str(), y.c_str()) < 0);
		}
	};
protected:
...
void ImageFormat::loadImage(CFile& file, unsigned int length)
{
	try {
		mem_alloc(length);
		void* buf = GlobalLock(m_hmem);
		unsigned int count = file.Read(buf, length);
		GlobalUnlock(m_hmem);
		_ASSERTE(count == length);
	}
	catch (...) {
		mem_free();
		throw;
